...
def test_int_default_name(trial: BaseTrial) -> None:
    d = {"param_int": "${ot.int:{low: -10, high: 10}}"}
    conf = OmegaTuna.create(d, trial=trial)

    assert isinstance(conf.param_int, int)
    assert conf.param_int == trial.suggest_int("param_int", -10, 10)


def test_int_yaml(trial: BaseTrial) -> None:
    s = """
param_int: ${ot.int:param_int, {low:-10, high:10}}
param_cat: ${ot.categorical:param_cat, {choices:[null, true, 1, 0.3, test]}}"""
    conf = OmegaTuna.create(s, trial=trial)
...
def test_float_default_name(trial: BaseTrial) -> None:
    d = {"param_float": "${ot.float: {low: -10.0, high: 10.0}}"}
    conf = OmegaTuna.create(d, trial=trial)

    assert isinstance(conf.param_float, float)
    assert conf.param_float == trial.suggest_float("param_float", -10, 10)
...
def test_categorical_default_name(trial: BaseTrial) -> None:
    d = {"param_cat": "${ot.categorical: {choices: [null, true, 1, 0.3, test]}}"}
    conf = OmegaTuna.create(d, trial=trial)
...
def test_resolve(trial: BaseTrial) -> None:
    d = {"param": "${ot.int: param_int, {low: -10, high: 10, default: -1}}"}
    conf1 = OmegaTuna.create(d, trial=trial)
    OmegaTuna.resolve(conf1)

    conf2 = OmegaTuna.create(d)
